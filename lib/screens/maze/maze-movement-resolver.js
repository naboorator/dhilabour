class MazeMovementResolver {
    ...
    tryToMoveItem(itemToMove, itemsOnAway, nextPosY, nextPosX) {
            ...
            if (itemsOnAway.size > 0) {
                ...
                itemsOnAway.forEach((item) => {
                    ...
                    if (item.hasAbility(ItemAbilities.CanInteract) && itemToMove.hasAbility(ItemAbilities.CanStartInteract)) {
                        canMoveToTile = false;
                        item.onInteract(this.gameBoardGrid, item);
                    }
                    ...
                    if (item.hasAbility(ItemAbilities.IsTrigger)) {
                        canMoveToTile = true;
                        if (itemToMove.hasAbility(ItemAbilities.CanTriggerTriggers)) {
                            if (item['onTrigger']) {
                                item.onTrigger(this.gameBoardGrid, item);
                            } else {
                                console.error('Item has IsTrigger ability, but missing onTrigger method implementation')
                            }
                        }
                    }
                    ...
                    if (item instanceof BoxItem && itemToMove instanceof PlayerItem) {
        ...
        if (canMoveToTile) {
            ...
            if (this.gameBoardGrid.playableItemsMap.get(playGridKey).values().find(item => {
    ...
    interactWithItem(item) {
        ...
        switch (item.orientation) {
            case Orientation.Left:
                ...
                possibleItems = this.gameBoardGrid.getItemsOnPosition(item.position.y, nextPosX);

                if (possibleItems.size > 0) {
                    this.handleInteractWithItem(item, possibleItems)
            ...
            case Orientation.Up:
                ...
                possibleItems = this.gameBoardGrid.getItemsOnPosition(nextPosY, item.position.x)
    ...
    handleInteractWithItem(avatar, possibleItems) {
        possibleItems.forEach(possItem => {
            ...
            //     // this.gameBoardGrid.effects.addEffect(new ExplodeEffect(possItem, true));
            ...
            if (possItem.hasAbility(ItemAbilities.CanInteract) && avatar.hasAbility(ItemAbilities.CanStartInteract)) {
                possItem.onInteract(this.gameBoardGrid, possItem);
            }
    ...
    forceMoveLeft(item, timesToMove = 3) {
        item.onMoveLeft();
        item.abilities = [ItemAbilities.LivesFootsteps]
        let movedTimes = 0;
        let interval = setInterval(() => {
            if (movedTimes < timesToMove) {
                ...
                nextPosX = nextPosX < 0 ? this.gameBoardGrid.maxLineLengths[posY] : nextPosX;
                this.gameBoardGrid.removeItemFromTile(item);
                this.gameBoardGrid.effects.addEffect(new FootstepEffects(item, item.orientation));
                this.gameBoardGrid.setPlayableItemPosition(posY, nextPosX, item);

                let nearbyTiles = this.gameBoardGrid.findNearByTiles(item, 1);
                this.gameBoardGrid.makeVisible(nearbyTiles);
    ...
    forceMoveRight(item, timesToMove = 3) {
        item.onMoveRight();
        item.abilities = [ItemAbilities.LivesFootsteps]
        let movedTimes = 0;
        let interval = setInterval(() => {
    ...
    forceMoveUp(item, timesToMove = 3) {
        item.onMoveRight();
        item.abilities = [ItemAbilities.LivesFootsteps]
        let movedTimes = 0;
        let interval = setInterval(() => {
            if (movedTimes < timesToMove) {
                ...
                this.gameBoardGrid.removeItemFromTile(item);
                this.gameBoardGrid.effects.addEffect(new FootstepEffects(item, item.orientation));
                this.gameBoardGrid.setPlayableItemPosition(nextPosY, posX, item);
    ...
    forceMoveDown(item, timesToMove = 3) {
        item.onMoveRight();
        item.abilities = [ItemAbilities.LivesFootsteps]
        let movedTimes = 0;
        let interval = setInterval(() => {
