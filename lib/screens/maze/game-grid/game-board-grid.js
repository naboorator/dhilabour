class GameBoardGrid {
    ...
    parseMap(layout) {
        ...
        for (let y = 0; y < rows.length; y++) {
            ...
            for (let x = 0; x < rows[y].length; x++) {
                ...
                this.mazeTiles.set(getGridKey(mazeTile.y, mazeTile.x), mazeTile);
    ...
    onFocus(startTile) {
        ...
        if (item) {
            ...
            //this.game.player.setPosition(y, x)
    ...
    setPlayableItemPosition(y, x, item) {
        const tile = this.mazeTiles.get(getGridKey(y, x));
        if (tile) {
            if (item.setPosition(y, x)) {
                tile.addItem(item);
            }
    ...
    getItemsOnPosition(y, x) {
        let tile = this.mazeTiles.get(getGridKey(y, x));
        if (tile) {
    ...
    removeItemFromTile(itemToRemove) {
        const key = getGridKey(itemToRemove.position.y, itemToRemove.position.x);
    ...
    destroyEnemy(avatar) {
        ...
        this.effects.addEffect(new RotateEffect(this, avatar, 10,));
    }
