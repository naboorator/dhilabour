...
<style>  
    ...
    <div class="maze-screen">
        <div style="display: flex; align-content: flex-end;  justify-content: center;">
            ...
            <div class="maze-bg" style="overflow:hidden; width:{{cameraWidth}}px; height: {{cameraHeight}}px; position:relative; text-align: center">{{maze}}</div>
    ...
    processLevel(level) {
        ...
        this.gameBoardGrid.camera.getCameraTiles().forEach(cameraTile => {
            ...
            let style = [
                'position: absolute',
                'left: ' + cameraTile.cameraX * TileSize + 'px',
                'top: ' + cameraTile.cameraY * TileSize + 'px',
                'height: ' + TileSize + 'px',
                'width: ' + TileSize + 'px',
            ]
            this.mazeOutput += '<div class="cell-wrapper" style="' + style.join(';') + '"  title="' + getGridKey(cameraTile.y, cameraTile.x) + '">';
        ...
        try {
            ...
            this.gameBoardGrid.onReady.subscribe((isReady) => {
                if (isReady) {
                    ...
                    if (area.config.backgroundMusic) {
                        this.backgroundMusic = new Sound(area.config.backgroundMusic);

                        this.backgroundMusic.volume(0.1).repeat().play();
                    }
