    ...
    * @var  PlayerItem
    ...
    chekBoxPlaces() {
        this.boxPlaces.forEach((boxPlace) => {
            const item = this.getItemOnPosition(boxPlace.position.y, boxPlace.position.x);
        ...
        if(totalCompleted === this.boxPlaces.length) {
            this.renderer.showCompleted('Bravo');
            this.pause = true;
    ...
    handleItemSwap(game,controller, controllerMethod, itemToMove, itemOnAway, nextPosY, nextPosX) {
       ...
       let isVerticalMovment = itemToMove.position.y != nextPosY ? true : false;
        ...
        if(itemOnAway instanceof MineItem &&  !(itemToMove instanceof MineItem)) {
                // LEt see it item can be pushed
                const hasMoved = controller[controllerMethod](itemOnAway);
                if(hasMoved) {
                    ++game.levelMoves
                    game.levelGrid[itemToMove.position.y][itemToMove.position.x] = replaceTile;
                    game.levelGrid[nextPosY][nextPosX] = itemToMove;
        ...
        if(itemOnAway instanceof BoxPlaceItem)  {
            ...
            if(itemToMove instanceof MineItem)  {
                ++game.levelMoves
                game.levelGrid[itemToMove.position.y][itemToMove.position.x] = replaceTile;
                game.levelGrid[nextPosY][nextPosX] = itemToMove;
                itemToMove.position.y = nextPosY;
