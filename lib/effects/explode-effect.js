class ExplodeEffect extends BaseEffect {
    ...
    constructor(item, isRoot = true, particles, minSpeed, maxSpeed) {
        ...
        this.itemToExplode = item;
        this.class = this.itemToExplode.class;
        this.createDuplicate(item);
        this.isRoot = isRoot;
        ...
        // this.yMovingUp = Math.random() <= 0.5;
        // this.xMovingLeft = Math.random() <= 0.5;


    }

    createDuplicate(item) {
        this.duplicate = JSON.parse(JSON.stringify(item));
    }
    ...
    prepareEffect() {
        if (this.isRoot && this.particles > 0) {
            ...
            while (i <= this.particles) {
                this.addEffect(new ExplodeEffect(this.itemToExplode, false))
                i++;
            }
        }
